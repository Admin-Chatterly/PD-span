...
/**
 * The intel log for a person, organization or case: composer on top, newest
 * first below. Links to the other things a note is attached to, skipping the
 * page it is shown on.
 */
          ...
          <ol className="flex flex-col divide-y divide-border/60">
            {notes.map((note) => {
              ...
              if (note.organization && note.organization.id !== target.organizationId) {
                related.push({ href: `/organizations/${note.organization.id}`, label: note.organization.name })
              }
