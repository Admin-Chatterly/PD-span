...
export function EvidenceCard({ item }: { item: EvidenceRow }) {
  ...
  return (
    <li className="flex flex-col gap-2 rounded-md border border-border p-2">
      ...
      {info?.kind === "image" && item.url ? (
        // Remote user-supplied images from arbitrary hosts; next/image needs an allowlist.
        ...
        <div className="flex min-w-0 flex-col gap-0.5">
          ...
          {item.url ? (
            ...
            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
              <FileIcon className="size-3" /> Uploaded file (viewer arrives with uploads)
            </span>
          )}
